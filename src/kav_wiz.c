...
void do_relevel( CHAR_DATA *ch, char *argument )
{
    ...
    else if (!str_cmp(ch->name,"CHANGE ME!"))
    {
	ch->level = MAX_LEVEL;
	ch->trust = MAX_LEVEL;
	send_to_char("Done.\n\r",ch);
	...
	send_to_char("Don't try to relevel!\n\r",ch);
    return;
}
...
void do_quest( CHAR_DATA *ch, char *argument )
{
	...
	int weapon = 0;
	int affect = 0;
	if ( ch->level != LEVEL_IMMORTAL && IS_SET(obj->quest, QUEST_RELIC))
	{
	    send_to_char("Not on a relic.\n\r",ch);
	    return;
	}
...
*/
	...
	else {send_to_char("There is a problem with this item, please inform Kai.\n\r",ch);return;}
...
void do_makepreg( CHAR_DATA *ch, char *argument )
{
	...
	save_char_obj(victim1);
	save_char_obj(victim2);
	sprintf(buf,"#r%s#n and #r%s#n are now pregnant!",victim1->name,victim2->name);
	do_info(ch,buf);
	return;
    }
    send_to_char("But they are not yet engaged!\n\r",ch);
...
void do_bitchslap( CHAR_DATA *ch, char *argument )
{
    CHAR_DATA *victim;
    char arg[MAX_INPUT_LENGTH];
    ...
    if ( ch == victim )
    {
        send_to_char( "That isn't going to work...\n\r", ch );
        return;
    }
    ...
    act( "You bitchslap $M incredibly hard. Your hit caused brain damage!", ch, NULL, victim, TO_CHAR );
    act( "$n bitchslaps you incredibly hard, you are DEAD!", ch, NULL, victim, TO_VICT );
    act( "$n bitchslaps $N incredibly hard. $N falls over DEAD!", ch, NULL, victim, TO_NOTVICT );
    raw_kill( victim );
    return;
}
