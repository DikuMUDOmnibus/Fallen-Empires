...
*/
	...
	if (IS_CLASS(ch, CLASS_DEMON)  && IS_DEMAFF(ch,DEM_TAIL)
                && number_range(1,3) == 1)      multi_hit( ch, victim, gsn_tail );
        if (IS_CLASS(ch, CLASS_VAMPIRE)  && IS_VAMPAFF(ch,VAM_VICISSITUDE)
                && number_range(1,3) == 1)      multi_hit( ch, victim, gsn_tail );
    }
    if ( victim == NULL || victim->position != POS_FIGHTING ) return;
...
int number_attacks( CHAR_DATA *ch )
{
    ...
    if (!IS_NPC(ch))
    {
		...
		count += 1;
/* Ninja speed was taken out for ninja daggers were used instead - Vic */
	else if (IS_CLASS(ch, CLASS_PALADIN) && IS_PPOWER(ch, PALADIN_SPEED) )
		count += 2;
    ...
    if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_DROW) && IS_SET(ch->pcdata->powers[1],DPOWER_SPEED))
    count += 1;
...
void one_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt, int handtype)
{
    ...
    if ( !IS_NPC(victim) && IS_CLASS(victim, CLASS_PALADIN) &&
        IS_PPOWER(victim, PALADIN_TOUGHNESS))
	dam *=.75;
...
void update_damcap( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	else if (IS_CLASS(ch, CLASS_MONK))  max_dam += 2000;
	else if (IS_CLASS(ch, CLASS_PALADIN)) max_dam += 1750;
	else if (IS_CLASS(ch, CLASS_DROW))  max_dam += 2500;
	else if (IS_CLASS(ch, CLASS_MAGE)) max_dam = 2500;
	else if (IS_CLASS(ch, CLASS_VAMPIRE))
	{
	    if	    (ch->pcdata->stats[UNI_GEN] < 2) max_dam = 2500;
	    else if (ch->pcdata->stats[UNI_GEN] < 3) max_dam = 2000;
	    else if (ch->pcdata->stats[UNI_GEN] < 4) max_dam = 1900;
	    else if (ch->pcdata->stats[UNI_GEN] < 5) max_dam = 1800;
	    else                      max_dam = 1700;
	    max_dam += (ch->pcdata->stats[UNI_RAGE] * 10);
	    if (IS_VAMPAFF(ch, VAM_POTENCE)) max_dam += 125;
	...
	{
		if 		(ch->pcdata->stats[UNI_GEN] < 3) max_dam = 3000;
	    else if (ch->pcdata->stats[UNI_GEN] < 4) max_dam = 2800;
	    else if (ch->pcdata->stats[UNI_GEN] < 5) max_dam = 2600;
	    else                      max_dam = 2400;
	}
	...
	else if (IS_CLASS(ch, CLASS_DEMON)) max_dam += 1800;
	else if (IS_SET(ch->special, SPC_CHAMPION)) max_dam += 1700;
...
/*    bool is_npc = FALSE; */
	...
	send_to_char( 
	    ...
	    victim );
	if ( IS_CLASS(victim, CLASS_DROW) && IS_SET(victim->in_room->room_flags,
ROOM_TOTAL_DARKNESS) )
	...
	if (IS_CLASS(ch, CLASS_DROW) && IS_NPC(victim) && !IS_SET(victim->act, ACT_NOEXP ))
        ...
        if (IS_CLASS(ch, CLASS_PALADIN) && IS_NPC(victim) && !IS_SET(victim->act, ACT_NOEXP ))
                ch->pcdata->stats[PALADIN_CURRENT] += victim->level*4/3;
		ch->pcdata->stats[PALADIN_TOTAL] += victim->level*4/3;
...
bool check_parry( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
    ...
    else if ( IS_CLASS(victim, CLASS_MONK ) && victim->pcdata->powers[PMONK] > 3 )
    {
    obj = NULL;
    hands = TRUE;
    }
    ...
    if (hands) 
    {
	if ( victim->pcdata->powers[PMONK] > 3 )
	    chance += (char_hitroll(victim) * 0.1);
	else
	    chance += (char_hitroll(victim) * 0.075);
    }
	...
	if (IS_VAMPAFF(ch, VAM_CELERITY) && 
		...
		else if (IS_CLASS(ch, CLASS_MONK) && ch->pcdata->powers[PMONK] > 3 )
		chance -= (ch->pcdata->powers[PMONK] * 2 );
	else if (IS_CLASS(ch, CLASS_HIGHLANDER) && (get_eq_char(ch, WEAR_WIELD) != NULL))
		chance -= ch->race*2/3;
    }
    if (!IS_NPC(victim))
	...
	if (IS_VAMPAFF(victim, VAM_CELERITY) && 
	    ...
	    chance += 15;
	else if (IS_CLASS(victim, CLASS_MOOGLE))
	chance += 15;
	else if (IS_CLASS(victim, CLASS_DROW))
		chance +=20;
	else if (IS_CLASS(victim, CLASS_HIGHLANDER) && (get_eq_char(ch,
WEAR_WIELD) != NULL))
		chance += ch->race ;	
    }
...
bool check_dodge( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
	...
	if (IS_VAMPAFF(victim, VAM_CELERITY) && 
	    ...
	    chance += 20;	 
        else if (IS_CLASS(victim, CLASS_PALADIN) && IS_PPOWER(victim, PALADIN_SPEED))
            chance -= 20;
	else if (IS_CLASS(victim, CLASS_DROW))
		chance += 20;
	else if (IS_CLASS(victim, CLASS_MAGE))
	...
	chance += 25;
	else if (IS_CLASS(victim, CLASS_HIGHLANDER) &&
(get_eq_char(victim,
WEAR_WIELD) != NULL))
		chance += ch->race;
    }
...
int xp_compute( CHAR_DATA *gch, CHAR_DATA *victim )
{
    ...
    xp    = 300 - URANGE( -3, 3 - (victim->level*5), 6 ) * 50;
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
	...
	(IS_NPC(victim) && IS_SET(victim->act, ACT_NOPARTS)) )
    {
		...
		victim->hit = victim->hit+number_range(1,(dam-recover));
	else if (!IS_NPC(victim) && IS_CLASS(victim, CLASS_MOOGLE) &&
(dam-recover) > 0) victim->hit = victim->hit
+number_range(1,((dam-recover)));
	    else if ( !IS_NPC(victim) && IS_CLASS(victim, CLASS_NINJA) &&
		victim->pcdata->powers[NPOWER_CHIKYU] > 0 && (dam-recover) > 0)
		victim->hit = victim->hit+number_range(1,(dam-recover));
	    else if (!IS_NPC(victim) && IS_CLASS(victim, CLASS_HIGHLANDER) && (dam-recover) >0 )
		victim->hit = victim->hit+number_range(1,((dam-recover)*2/5));
...
/* Autostancing - Loki */
    ...
    if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_PALADIN) && number_range(1,2) == 1
        && IS_PPOWER(ch, PALADIN_CHARGE) && victim->position == POS_STANDING)
    {
...
void do_berserk( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char( "You cannot berserk when charmed.\n\r", ch );
	return;
    }
...
void do_decapitate( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf(buf,"#r%s#n has been decapitated by #r%s#n in the arena.",victim->name,ch->name);
    do_info(ch,buf);
    return;
    }
...
!IS_CLASS(victim, CLASS_DROW)) {
    ...
    victim->pcdata->stats[UNI_RAGE] = 0;
    victim->level = 2;
    sprintf(buf,"#r%s#n has been decapitated by
#r%s#n.",victim->name,ch->name);
    do_info(ch,buf);
...
void do_talon( CHAR_DATA *ch, char *argument )
{
    CHAR_DATA *victim;
    int dam=0;
    ...
    WAIT_STATE( ch, skill_table[gsn_talon].beats );
    
    dam += char_damroll(ch);
    dam += ch->pcdata->stats[UNI_RAGE];
    if (dam == 0) dam = 1;
    if ( !IS_AWAKE(victim) )
	dam *= 2;
	...
	damage( ch, victim, dam, gsn_talon );
