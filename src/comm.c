...
# define min(a,b)     (((a) < (b)) ? (a) : (b))
...
#if defined(macintosh) || defined(MSDOS)
    boot_db( );
    log_string( "Fallen Empires is ready to rock." );
    game_loop_mac_msdos( );
#endif
    ...
    boot_db();
    sprintf( log_buf, "Fallen Empires is ready to rock on port %d.", port );
    log_string( log_buf );
    if (fCopyOver)
        copyover_recover();
...
#define WE_WANNA_BAN
    ...
    for ( pban = ban_list; pban != NULL; pban = pban->next )
    {
        if ( !str_suffix( pban->name, dnew->host ) )
        {
            ...
            close( desc );
                sprintf(log_buf,"Access denied to site %s\n\r",pban->name);
        log_string(log_buf);
...
/*
	...
	** Avoid nasty duplication bug - KaVir */
    ...
    case CON_READ_MOTD:
	write_to_buffer( d, "\n\r#lWelcome to Fallen Empires.#n\n\r",
	    0 );
	ch->next	= char_list;
	char_list	= ch;
	d->connected	= CON_PLAYING;
...
*/
	if ( ch->level == 0 )
	{
	    ...
	    do_look( ch, "auto" );
/* Black Shimmering Equipment -> Character */
/* By: Kai                                 */
    ...
    obj_to_char(create_object(get_obj_index( 30343 ), 1), ch);
    send_to_char("\n\r", ch );
    send_to_char("A veteran warrior gives you a set of black shimmering armor.\n\r",ch);
    send_to_char("HINT... Type: align evil\n\r",ch);
    send_to_char("\n\r",ch);
	...
	sprintf(buf,"#r%s#n has entered Fallen Empires.", ch->name);
	do_info(ch,buf);
