...
void do_chant( CHAR_DATA *ch, char *argument )
{
    ...
    act("$n chants some arcane words from $p.",ch,book,NULL,TO_ROOM);
        if (IS_SET(page->value[3], AFF_SHADOWPLANE) && ch->position == POS_FIGHTING)
        {
            send_to_char("Not while fighting!\n\r", ch);
            return;
        }
        if (IS_SET(page->value[3], AFF_ETHEREAL) && ch->position == POS_FIGHTING)
        {
            send_to_char("Not while fighting!\n\r", ch);
            return;
        }
...
void do_magearmor( CHAR_DATA *ch, char *argument )
{
    ...
    if (arg[0] == '\0')
    {
	send_to_char("Please specify which piece of mage armor you wish to make: Visor, Cloak, Girth.\n\r",ch);
	return;
    }
    ...
    if      (!str_cmp(arg,"visor"     )) vnum = 29703;
    else if (!str_cmp(arg,"cloak"   )) vnum = 29704;
    else if (!str_cmp(arg,"girth"    )) vnum = 29705;
    else
    {
	send_to_char("Please specify which piece of mage armor you wish to make: Visor, Cloak, Girth.\n\r",ch);
	return;
