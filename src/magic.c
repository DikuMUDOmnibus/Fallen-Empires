...
&& (dtype == 2 || dtype == 3)) &&
        ...
        !(IS_CLASS(ch, CLASS_DROW) && ch->pcdata->stats[UNI_GEN] == 2)
) return;
...
void spell_faerie_fog( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	REMOVE_BIT   ( ich->affected_by, AFF_DARKNESS	);
...
void spell_guardian( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    victim->level = level;
    victim->hit = 500*level;
    victim->max_hit = 500*level;
...
void spell_brew( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    {
	send_to_char( "Oh dear...big bug...please inform Kai.\n\r", ch );
	return;
    }
    obj->value[1] = sn;
...
void spell_find_familiar( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if (victim == NULL)
	{send_to_char("Error - please inform Kai.\n\r",ch); return;}
    }
    else if ( !str_cmp( target_name, "raven" ) )
    {
