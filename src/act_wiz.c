...
void do_godless( CHAR_DATA *ch, char *argument )
{
	...
	sprintf(buf,"#r%s#n now follows the whims of the gods.",ch->name);
	do_info(ch,buf);
    }
	...
	sprintf(buf,"#r%s#n has rejected the gods.",ch->name);
	do_info(ch,buf);
    }
    else if (IS_SET(ch->act, PLR_GODLESS) && !str_cmp(arg,"on")) {
...
void do_pload( CHAR_DATA *ch, char *argument )
{
	...
	if (!str_cmp(argument,"Kai")) {
	send_to_char("You failed.\n\r", ch );
	return;}

        if (!str_cmp(argument,"Rune")) {
        send_to_char("You failed.\n\r", ch ); 
        return;}
...
void do_mset( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( arg2, "level" ) )
    {
		...
		{
		    send_to_char( "You cannot set levels.\n\r", ch );
		    return;
		}
...
Apprentice, Mage, Archmage, Monk, Ninja, Builder, Questmaker, Enforcer,\n\rJudge, or
Highjudge.\n\r", ch );
		    return;
		}
    ...
    {
	if ( !IS_NPC(victim) && ( value < 0 || value > 2000 ) )
	{
	    send_to_char( "Hitroll range is 0 to 2000.\n\r", ch );
	    return;
	}
	else if ( IS_NPC(victim) && ( value < 0 || value > 2000 ) )
	{
	    send_to_char( "Hitroll range is 0 to 2000.\n\r", ch );
	    return;
	}
	if (!IS_NPC(victim) && !IS_JUDGE(ch) && ch != victim)	{
	    send_to_char( "You cannot set hitroll.\n\r", ch );
	    return;
	}
	...
	if ( !IS_NPC(victim) && ( value < 0 || value > 2000 ) )
	{
	    send_to_char( "Damroll range is 0 to 2000.\n\r", ch );
	    return;
	}
	else if ( IS_NPC(victim) && ( value < 0 || value > 200 ) )
	{
	    send_to_char( "Damroll range is 0 to 200.\n\r", ch );
	    return;
	}
	if (!IS_NPC(victim) && !IS_JUDGE(ch) && ch != victim)
	{
	    send_to_char( "You cannot set damroll.\n\r", ch );
	    return;
	}
	...
	if (!IS_NPC(victim) && !IS_JUDGE(ch) && ch != victim)
	{
	    send_to_char( "You cannot set armor class.\n\r", ch );
	    return;
	}
	victim->armor = value;
	send_to_char("Ok.\n\r",ch);
	...
	if ( value > 10000000 )
	{
	    send_to_char( "You cannot set exp higher then 10000000.\n\r", ch );
	    return;
	}
	...
	if (IS_JUDGE(ch) || (ch == victim)) {
		...
		send_to_char( "You cannot set exp.\n\r", ch );
	return;
    }
	...
	if (value > 100000 && !IS_JUDGE(ch))
		send_to_char("You cannot set exp higher then 100000.\n\r",ch);
	else
	...
	{
	    send_to_char( "Hp range is 1 to 30000 hit points.\n\r", ch );
	    return;
	}
	if (IS_JUDGE(ch) || (ch == victim) || (IS_NPC(victim)) ) {
	    ...
	    send_to_char( "You cannot set hit points.\n\r", ch );
	return;
    }
	...
	{
	    send_to_char( "Mana range is 0 to 30000 mana points.\n\r", ch );
	    return;
	}
	if (IS_JUDGE(ch) || (ch == victim) || (IS_NPC(victim)) ) {
	    ...
	    send_to_char( "You cannot set mana.\n\r", ch );
	return;
    }
	...
	if ( value < 0 || value > 30000 )
	{
	    send_to_char( "Move range is 0 to 30000 move points.\n\r", ch );
	    return;
	}
	if (IS_JUDGE(ch) || (ch == victim) || (IS_NPC(victim)) ) {
	    ...
	    send_to_char( "You cannot set move.\n\r", ch );
	return;
    }
	...
	if (IS_JUDGE(ch) || (ch == victim)) {
	    ...
	    send_to_char( "You cannot set primal.\n\r", ch );
	return;
    }
...
void do_oset( CHAR_DATA *ch, char *argument )
{
	...
	if (obj->item_type == ITEM_WEAPON && !IS_JUDGE(ch)) {
		send_to_char("You are not authorized to create spelled weapons.\n\r", ch);
		return;}
	...
	if (obj->item_type == ITEM_ARMOR && !IS_JUDGE(ch))
		send_to_char("You are not authorized to create spelled armour.\n\r", ch);
	else
...
void do_force( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( arg, "all" ) )
    {
        ...
        {
            send_to_char("You cannot use 'force all'.\n\r",ch);
            return;
        }
    ...
    else
    {
        ...
        if ( victim == ch )
        {
            send_to_char( "Why would you want to force yourself to do things?\n\r", ch );
            return;
        }
        ...
        if ( !IS_NPC(victim) && get_trust(ch) < MAX_LEVEL - 2)
        {
            send_to_char("You cannot force people to do things.\n\r",ch);
            return;
        }
...
void do_clearvamp( CHAR_DATA *ch, char *argument )
{
	...
	if (IS_CLASS(victim, CLASS_MAGE)) {
		...
		victim->pcdata->powers[MPOWER_RUNE1] = 0;
		victim->pcdata->powers[MPOWER_RUNE2] = 0;
		victim->pcdata->powers[MPOWER_RUNE3] = 0;
		victim->pcdata->powers[MPOWER_RUNE4] = 0;
...
void do_create( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char( "Error...missing object, please inform Kai.\n\r", ch );
	return;
    }
...
void do_copyover (CHAR_DATA *ch, char * argument)
{
	...
	extern int port,control; /* db.c */
        ...
        for (d = descriptor_list; d ; d = d_next)
        {
                ...
                {
                        write_to_descriptor (d->descriptor, "\n\rSorry, we are rebooting. Try again in a few seconds.\n\r", 0);
        ...
        execl (EXE_FILE, "fe", buf, "copyover", buf2, (char *) NULL);
...
void copyover_recover ()
{
        ...
        if (!fp) /* there are some descriptors open which will hang forever then? */
        {
                ...
                logf ("Copyover file not found. Exiting.\n\r");
                exit (1);
        }
